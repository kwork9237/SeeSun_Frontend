...
import { ChevronLeft, ChevronRight, Video, Star, Search, Calendar, BookOpen } from "lucide-react";
          ...
          {weekDates.map((dateObj, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* 스케줄이 있으면 파란 점 표시 */}
                <div
                  className={`w-1.5 h-1.5 rounded-full mt-1 ${hasSchedule && !isSelected ? "bg-blue-400" : "bg-transparent"}`}
                ></div>
              </button>
            );
          })}
        ...
        <div className="space-y-3">
          {filteredSchedules.length > 0 ? (
            filteredSchedules.map((schedule, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 hover:border-blue-200 transition-colors group shadow-sm"
              >
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {myLectures.map((lecture) => (
            <div
              key={lecture.leId}
              className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 flex flex-col h-full"
            >
              {/* 카드 상단: 아이콘 및 평점 */}
              ...
              <div className="pt-4 border-t border-gray-100 mt-auto">
                <span className="text-xs text-gray-400 font-medium">
                  신청일:{" "}
                  {lecture.modifiedAt ? String(lecture.modifiedAt).split("T")[0] : "날짜 없음"}
                </span>
              </div>
            </div>
          ))}
