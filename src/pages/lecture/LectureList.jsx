        ...
        <div className="flex gap-8 items-start">
          ...
          <aside className="w-80 flex-shrink-0 sticky top-24">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="space-y-2">
                  {languages.map((lang) => (
                    <label key={lang} className="flex items-center cursor-pointer group">
                      <input type="radio" name="language" checked={selectedLanguage === lang} onChange={() => setSelectedLanguage(lang)} className="w-4 h-4 accent-orange-500" />
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  {[{ label: '쉬움', value: 1 }, { label: '보통', value: 2 }, { label: '어려움', value: 3 }].map((level) => (
                    <button key={level.value} onClick={() => setSelectedLevel(selectedLevel === level.value ? '' : level.value)}
                      className={`flex-1 py-1.5 text-sm border rounded transition-colors ${selectedLevel === level.value ? 'bg-orange-500 text-white border-orange-500' : 'text-gray-400 hover:bg-gray-50'}`}>
                      {level.label}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={handleApplyFilters} disabled={loading} className="w-full py-3.5 bg-blue-500 text-white rounded-xl hover:bg-blue-600 font-bold shadow-md shadow-blue-100 disabled:opacity-50">
          ...
          <main className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {lectures.map((lecture) => (
                <div key={lecture.leId} className="bg-white rounded-[24px] border border-gray-100 shadow-sm flex flex-col h-full hover:shadow-lg transition-all overflow-hidden group">
                  <div className="p-6 flex-grow">
                    <div className="flex items-start mb-4">
                      <div className="w-16 h-16 bg-[#D9D9D9] rounded-full mr-4 flex-shrink-0 overflow-hidden">
                        {lecture.profileIcon && <img src={lecture.profileIcon} alt={lecture.instructorName} className="w-full h-full object-cover" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-lg text-gray-900 mb-1 truncate">{lecture.title}</h3>
                        <p className="text-sm text-gray-400 mb-2 font-medium">{lecture.instructorName || 'Unknown Mentor'}</p>
                        <div className="flex items-center">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} size={14} className={`mr-0.5 ${i < Math.floor(lecture.avgScore || 0) ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200'}`} />
                          ))}
                          <span className="text-xs font-bold text-gray-300 ml-1">({lecture.avgScore?.toFixed(1) || '0.0'})</span>
                        </div>
                      </div>
                    </div>

                    <p className="text-[11px] text-blue-500 font-bold mb-3 uppercase tracking-tighter">Native {lecture.categoryName} Speaker</p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      <span className="text-[10px] bg-orange-500 text-white px-2 py-0.5 rounded font-bold">#{lecture.categoryName}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${getDifficultyColor(lecture.difficulty)}`}>{getDifficultyLabel(lecture.difficulty)}</span>
                    </div>
                    <p className="text-xs text-gray-400 leading-relaxed line-clamp-2">{lecture.content}</p>
                  </div>

                  <div className="px-6 pb-6 mt-auto">
                    <div className="pt-4 border-t border-gray-50">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[11px] font-bold text-gray-400 tracking-tight">총 시간 : {lecture.totalHours || 0}시간</span>
                        <span className="text-orange-500 text-[11px] font-black uppercase">Credit N/hour</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-[11px] font-bold text-gray-400 tracking-tight">주요시간 : {lecture.availableTime || '시간 협의'}</span>
                        <button 
                          onClick={() => navigate(`/lecture/detail/${lecture.leId}`)}
                          className="bg-orange-500 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm hover:bg-orange-600 active:scale-95 transition-all"
                        >
                          강의 신청
                        </button>
                      </div>
                      <div className="mt-2 text-right">
                        <span className="text-2xl font-black text-orange-500">₩{(lecture.cost || 0).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
