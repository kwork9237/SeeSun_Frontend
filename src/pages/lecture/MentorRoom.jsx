...
const MentorRoom = ({ isMentor }) => {
    ...
    const handleStart = async () => {
        ...
        if (data) {
            ...
            const finalNickname = isMentor ? data.nickname : `${data.nickname}_${Math.floor(Math.random() * 1000)}`;
        ...
        <div style={{ padding: '20px', fontFamily: 'Arial' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
                {!isStarted ? (
                    ...
                    <button onClick={stopSession} style={{ ...btnStyle, backgroundColor: '#ff4d4d' }}>회의 종료</button>
                )}
            </div>
            <div style={{ display: 'flex', gap: '20px' }}>
                <div style={{ flex: 3, background: '#1a1a1a', borderRadius: '10px', overflow: 'hidden', position: 'relative' }}>
                    <video ref={mentorVideoRef} autoPlay playsInline style={{ width: '100%', height: '500px', objectFit: 'contain' }} />
                    <div style={{ position: 'absolute', top: 10, left: 10, color: '#fff', background: 'rgba(0,0,0,0.5)', padding: '5px 10px' }}>
                ...
                <div style={{ flex: 1, border: '1px solid #ddd', borderRadius: '10px', padding: '15px' }}>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0 }}>
                        {/* 1. 나 자신은 무조건 표시 */}
                        <li style={{ padding: '10px 0', borderBottom: '2px solid #4CAF50', fontWeight: 'bold', color: '#4CAF50' }}>
                            👤 {myInfo.nickname} (나)
                        </li>
