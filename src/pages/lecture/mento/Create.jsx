...
const Create = () => {
  ...
  const handleCreateLecture = async () => {
    ...
    try {
      ...
      // 2. 주소에 8080 추가 및 headers에 토큰 실기
...
const BasicInfo = ({ data, setData, errors }) => (
  <div className="space-y-6">
    ...
    <div className="space-y-2">
      ...
      <input type="text" placeholder="강의제목 입력" className={`w-full p-4 bg-gray-50 rounded-2xl outline-none focus:ring-2 ${errors.title ? 'ring-2 ring-red-500 bg-red-50' : 'focus:ring-orange-500'}`} value={data.title} onChange={(e) => setData({title: e.target.value})} />
    ...
    <div className="grid grid-cols-2 gap-6">
      <div className="space-y-2">
        ...
        <select className="w-full p-4 bg-gray-50 rounded-2xl outline-none" value={data.language} onChange={(e) => setData({language: e.target.value})}><option value="en">영어</option><option value="jp">일본어</option><option value="cn">중국어</option></select>
      </div>
      <div className="space-y-2">
        <label className="block text-sm font-bold text-gray-700">난이도</label>
        <select className="w-full p-4 bg-gray-50 rounded-2xl outline-none" value={data.level} onChange={(e) => setData({level: e.target.value})}><option value="1">쉬움</option><option value="2">보통</option><option value="3">어려움</option></select>
      </div>
    </div>
    <div className="space-y-2">
      <label className="block text-sm font-bold text-gray-700">강의 상세내용</label>
      <textarea rows="5" placeholder="강의 상세내용 입력" className={`w-full p-4 bg-gray-50 rounded-2xl outline-none focus:ring-2 ${errors.description ? 'ring-2 ring-red-500 bg-red-50' : 'focus:ring-orange-500'}`} value={data.description} onChange={(e) => setData({description: e.target.value})} />
            ...
            {section.lessons.map((lesson) => (
              <div key={lesson.id}>
                <div className={`flex items-center gap-3 p-4 bg-white border rounded-2xl group transition-all shadow-sm ${errors[`lesson_${lesson.id}`] ? 'border-red-500 bg-red-50' : 'border-gray-100 hover:border-orange-200'}`}>
                  ...
                  <div className="flex items-center bg-gray-100 px-3 py-1 rounded-xl"><input type="number" className="w-12 bg-transparent text-right text-xs font-bold outline-none" value={lesson.duration} onChange={(e) => updateData({ sections: data.sections.map(s => s.id === section.id ? {...s, lessons: s.lessons.map(l => l.id === lesson.id ? {...l, duration: e.target.value} : l)} : s) })} /><span className="text-[10px] ml-1 text-gray-500 font-bold">min</span></div>
...
const Schedule = ({ data, updateData, errors }) => {
  ...
  const generateSlots = () => {
    ...
    const slots = []; const dayIndices = data.selectedDays.map(d => (days.indexOf(d) + 1) % 7);
    let current = new Date(start);
    while(current <= end) { if(dayIndices.includes(current.getDay())) { slots.push(`${current.toLocaleDateString('en-US', {month:'short', day:'numeric'})} ${data.startTime}`); } current.setDate(current.getDate() + 1); }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div><label className="block text-sm font-bold text-gray-700 mb-2">강의 시작날짜</label><div className="relative"><Calendar className={`absolute left-4 top-4 ${errors.startDate ? 'text-red-400' : 'text-gray-400'}`} size={18} /><input type="date" className={`w-full p-4 pl-12 bg-gray-50 rounded-2xl outline-none focus:ring-2 ${errors.startDate ? 'ring-2 ring-red-500' : 'focus:ring-orange-500'}`} value={data.startDate} onChange={(e) => updateData({startDate: e.target.value})} /></div></div>
        <div><label className="block text-sm font-bold text-gray-700 mb-2">강의 종료날짜</label><div className="relative"><Calendar className={`absolute left-4 top-4 ${errors.endDate ? 'text-red-400' : 'text-gray-400'}`} size={18} /><input type="date" className={`w-full p-4 pl-12 bg-gray-50 rounded-2xl outline-none focus:ring-2 ${errors.endDate ? 'ring-2 ring-red-500' : 'focus:ring-orange-500'}`} value={data.endDate} onChange={(e) => updateData({endDate: e.target.value})} /></div></div>
      </div>
      <div className="space-y-3">
        <label className="block text-sm font-bold text-gray-700">요일 선택</label>
        <div className="flex gap-2">{days.map(d => (<button key={d} onClick={() => updateData({selectedDays: data.selectedDays.includes(d) ? data.selectedDays.filter(day => day !== d) : [...data.selectedDays, d]})} className={`flex-1 py-3 rounded-xl font-bold text-sm border transition-all ${data.selectedDays.includes(d) ? 'bg-orange-500 border-orange-500 text-white shadow-md shadow-orange-100' : 'bg-white border-gray-200 text-gray-400 hover:bg-gray-50'}`}>{d}</button>))}</div>
      ...
      <div className="p-6 bg-orange-50/50 rounded-3xl border border-orange-100 space-y-4">
        ...
        <div className="flex items-center gap-4"><input type="time" className="flex-1 p-3 rounded-xl border-none outline-none" value={data.startTime} onChange={(e) => updateData({startTime: e.target.value})} /><span className="text-gray-300">—</span><input type="time" className="flex-1 p-3 rounded-xl border-none outline-none" value={data.endTime} onChange={(e) => updateData({endTime: e.target.value})} /><div className="flex items-center gap-2 bg-white px-4 py-3 rounded-xl border border-orange-100"><span className="text-xs text-gray-400">학생수:</span><input type="number" className="w-8 outline-none font-bold text-sm text-orange-600" value={data.maxStudents} onChange={(e) => updateData({maxStudents: e.target.value})} /></div></div>
      </div>
      <div className="space-y-3">
        <label className="block text-sm font-bold text-gray-700">강의 시간 저장목록</label>
        <div className={`flex flex-wrap gap-2 p-4 bg-gray-50 rounded-2xl min-h-[60px] border ${errors.generatedSlots ? 'border-red-500 bg-red-50' : 'border-gray-100'}`}>{data.generatedSlots.length === 0 ? (<p className="text-xs text-gray-400 font-medium">위의 날짜 생성 버튼을 클릭하세요.</p>) : (data.generatedSlots.map((s, i) => (<div key={i} className="bg-white px-4 py-2 rounded-full text-xs font-bold border border-gray-200 flex items-center gap-2 shadow-sm transition-all hover:border-orange-300">{s} <button onClick={() => updateData({generatedSlots: data.generatedSlots.filter((_, idx) => idx !== i)})} className="text-gray-300 hover:text-red-500">×</button></div>)))}</div>
      </div>
      <div className="space-y-2">
        <label className="block text-sm font-bold text-gray-700">강의 가격</label>
        <div className="relative"><span className={`absolute left-5 top-4 font-black text-lg ${errors.price ? 'text-red-400' : 'text-gray-400'}`}>$</span><input type="number" placeholder="0" className={`w-full p-4 pl-10 bg-gray-50 rounded-2xl font-black text-xl outline-none focus:ring-2 ${errors.price ? 'ring-2 ring-red-500 bg-red-50' : 'focus:ring-orange-500'}`} value={data.price} onChange={(e) => updateData({price: e.target.value})} /></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4 text-orange-600"><Calendar size={18} /><span className="font-bold text-sm">강의 정보</span></div>
        <div className="space-y-1"><h3 className="text-xl font-black text-gray-800">{data.title || "강의 제목이 없습니다"}</h3><p className="text-sm text-gray-500">{data.language === 'en' ? '영어' : data.language} · 난이도 {data.level}</p></div>
      </div>
      <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-6 text-orange-600"><GripVertical size={18} /><span className="font-bold text-sm">강의 커리큘럼</span></div>
        <div className="grid grid-cols-2 gap-4 mb-6"><div className="bg-gray-50 rounded-2xl p-4 text-center"><p className="text-2xl font-black text-orange-600">{totalWeeks}</p><p className="text-[11px] font-bold text-gray-400 uppercase">주차별</p></div><div className="bg-gray-50 rounded-2xl p-4 text-center"><p className="text-2xl font-black text-orange-600">{totalLessons}</p><p className="text-[11px] font-bold text-gray-400 uppercase">실시간 강의</p></div></div>
        <div className="space-y-3">{data.sections.map((section, idx) => (<div key={section.id} className="flex justify-between items-center py-3 border-b border-gray-50 last:border-none"><span className="text-sm font-bold text-gray-700">{section.title || `Week ${idx + 1}`}</span><span className="text-xs text-gray-400">{section.lessons.length} 강의</span></div>))}</div>
      </div>
      <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4 text-orange-600"><Clock size={18} /><span className="font-bold text-sm">강의 스케쥴</span></div>
        <div className="flex items-center gap-2 text-gray-600 bg-gray-50 p-4 rounded-2xl"><Users size={16} className="text-gray-400" /><span className="text-sm font-medium">멘티 {data.maxStudents}명 · {data.generatedSlots.length}개 슬롯</span></div>
      </div>
      <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-2 text-orange-600"><span className="text-lg font-bold">$</span><span className="font-bold text-sm">총 강의 가격</span></div>
