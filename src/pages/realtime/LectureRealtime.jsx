...
const LectureRealtime = ({ lectureId }) => {
  ...
  const apiBootstrap = async () => {
    ...
    // fallback join
    const fb = await fetch("/api/seesun/janus/join", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ lectureId: effectiveLectureId }),
  ...
  const apiEnd = async (sessionId) => {
    const r = await fetch("/api/seesun/janus/end", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  const apiRecording = async (sessionId) => {
    const r = await fetch(`/api/seesun/janus/recording?sessionId=${sessionId}`, {
      credentials: "include",
    });
  ...
  const startSSE = (sessionId) => {
    const es = new EventSource(`/api/seesun/janus/events?sessionId=${sessionId}`, {
      ...
      <div style={{ display: "flex", gap: 20 }}>
        ...
        <div style={{ flex: 1, border: "1px solid #ddd", borderRadius: 10, padding: 10 }}>
          ...
          <ul style={{ listStyle: "none", padding: 0 }}>
            {myName && (
              <li
                style={{ padding: "8px 0", borderBottom: "2px solid #4CAF50", fontWeight: "bold" }}
              >
                👤 {myName} (나)
              </li>
            )}
